...
import 'editor/editor_surface.dart' show EditorRowBackgroundPainter;
...
class _MarkdownEditorWidgetState extends State<MarkdownEditorWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final editorTextStyle = GoogleFonts.getFont(
      provider.fontFamily,
    ...
    // 动态测量由当前字体与 Strut 约束决定的精准物理行高
    final linePainter = TextPainter(
      text: TextSpan(text: 'Wg', style: editorTextStyle),
      strutStyle: editorStrutStyle,
      textDirection: TextDirection.ltr,
    )..layout();
    final double effectiveLineHeight = linePainter.preferredLineHeight;
                    ...
                    child: Theme(
                      data: Theme.of(context).copyWith(
                        textSelectionTheme: TextSelectionThemeData(
                          selectionColor: isDark ? const Color(0xFFCBA6F7).withOpacity(0.3) : const Color(0xFF8839EF).withOpacity(0.3),
                          selectionHandleColor: isDark ? const Color(0xFFCBA6F7) : const Color(0xFF8839EF),
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Stack(
                              children: [
                                // 1. Grid Background Painter (if enabled)
                                if (provider.showGridLines)
                                  Positioned.fill(
                                    child: AnimatedBuilder(
                                      ...
                                      builder: (context, _) => CustomPaint(
                                        painter: EditorRowBackgroundPainter(
                                          ...
                                          charWidth: provider.fontSize * 0.6,
                                          horizontalOffset: provider.editorPadding,
                                          topPadding: provider.lineHeight * 16,
                                          lineHeight: effectiveLineHeight,
                                ...
                                // 2. Active Line Focused Highlight
                                if (provider.showLineHighlight)
                                  _buildActiveLineSlot(
                                    provider.lineHeight * 16,
                                    effectiveLineHeight,
                                    isDark,
                                    provider.cursorLine - 1,
                                  ),
                                      ...
                                      scrollbarTheme: ScrollbarThemeData(
                                        thumbColor: WidgetStateProperty.resolveWith((states) {
                                          if (states.contains(WidgetState.hovered) || states.contains(WidgetState.dragged)) {
                                            return (isDark ? const Color(0xFFCBA6F7) : const Color(0xFF1E66F5)).withOpacity(0.85);
                                          }
                                          return isDark ? Colors.white.withOpacity(0.12) : Colors.black.withOpacity(0.08);
                                        }),
  ...
  Widget _buildActiveLineSlot(double topPadding, double lineHeight, bool isDark, int activeLine) {
    return AnimatedBuilder(
      animation: _scrollController,
      builder: (context, child) {
         if (!_scrollController.hasClients) return const SizedBox.shrink();
         final offset = (topPadding + (activeLine * lineHeight)) - _scrollController.offset;
  ...
  void _showTableWizardDialog(BuildContext context, MarkdownProvider provider) {
    int rows = 3;
    int cols = 3;
    final isDark = Theme.of(context).brightness == Brightness.dark;
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text('${provider.t('rows')}: $rows', style: GoogleFonts.inter()),
                      Row(
                        children: [
                          IconButton(
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('${provider.t('cols')}: $cols', style: GoogleFonts.inter()),
                      Row(
                        children: [
                          IconButton(
                            icon: const Icon(Icons.remove),
              ...
              actions: [
                TextButton(
                  ...
                  child: Text(provider.t('cancel'), style: GoogleFonts.inter(color: Colors.grey)),
                ),
                ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    provider.insertSnippet(sb.toString(), '', provider.selectionStart, provider.selectionEnd);
                    Navigator.pop(context);
                  },
                  child: Text(provider.t('create'), style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
                ),
              ],
            );
          },
  ...
  Widget _buildToolbar(MarkdownProvider p, bool isDark) {
    ...
    return Container(
      ...
      child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
        ...
        _tool(Icons.grid_on_rounded, () => _showTableWizardDialog(context, p), p.t('table_wizard'), iconCol),
