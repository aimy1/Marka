...
class _SidebarWidgetState extends State<SidebarWidget> {
  int _activeTab = 0; // 0: Files, 1: Outline
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Header Tab Switcher (Files / Outline)
          Padding(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: _buildTabButton(0, provider.t('files'), Icons.folder_open_outlined, isDark),
                  ),
                  Expanded(
                    child: _buildTabButton(1, provider.t('outline'), Icons.format_list_bulleted_rounded, isDark),
                  ),
                ],
              ),
            ),
          ...
          Expanded(
            child: _activeTab == 0
                ? (provider.workspacePaths.isEmpty
                ...
                : _buildOutlineView(provider, isDark),
          ),
        ],
      ),
    );
  ...
  Widget _buildTabButton(int index, String label, IconData icon, bool isDark) {
    ...
    final activeColor = isDark ? const Color(0xFFCBA6F7) : const Color(0xFF1E66F5);
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected 
              ...
              : Colors.transparent,
          borderRadius: BorderRadius.circular(6),
          boxShadow: isSelected ? [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 2, offset: const Offset(0, 1))] : [],
        ),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, size: 14, color: isSelected ? activeColor : (isDark ? Colors.white38 : Colors.black45)),
  ...
  Widget _buildOutlineView(MarkdownProvider provider, bool isDark) {
    final outline = provider.getOutline();
    if (outline.isEmpty) {
      return Center(
        child: Text(
          provider.t('no_results'),
          style: GoogleFonts.inter(
            fontSize: 13,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final double leftPadding = (item.level - 1) * 12.0 + 8.0;
        final double fontSize = item.level == 1 ? 13 : 12;
        final FontWeight fontWeight = item.level == 1 ? FontWeight.bold : FontWeight.normal;
        
        return InkWell(
          onTap: () => provider.jumpToOffset(item.offset),
          borderRadius: BorderRadius.circular(6),
          child: Padding(
            padding: EdgeInsets.fromLTRB(leftPadding, 6, 8, 6),
            child: Row(
              children: [
                Text(
                  '#' * item.level,
