...
./appimagetool-x86_64.AppImage build/AppDir build/Marka-3.3.11-x86_64.AppImage || {
  ./appimagetool-x86_64.AppImage --appimage-extract
  ./squashfs-root/AppRun build/AppDir build/Marka-3.3.11-x86_64.AppImage
}

if [ -f build/Marka-3.3.11-x86_64.AppImage ]; then
  cp build/Marka-3.3.11-x86_64.AppImage dist/
  echo "AppImage created successfully at dist/Marka-3.3.11-x86_64.AppImage"
fi
...
if command -v dpkg-deb &> /dev/null; then
  dpkg-deb --build build/deb dist/Marka-3.3.11-amd64.deb || true
  echo "DEB created successfully at dist/Marka-3.3.11-amd64.deb"
else
